...
namespace CSMaklerbuero
{
	public class Verwaltung
	{
		...
		double tempd;
		Objekt tempObj;
		string temp; /*variable für temporäre Zwischenspeicherungen*/
		/*TEMPORARIES*/
		...
		public void newObjekt(){
			...
			if (!objListe.Any() /*if list doesn't contain anything*/) {
				objnr = 1;
			} else {
				objnr = objListe.Last ().getObjNr () + 1; /*increase number in list for new Objekt*/
			}
			...
			if (!temp.Any (x => char.IsLetter (x))) {/*true = keine Buchstaben*/
				f = double.Parse (temp);
			} else { /*buchstaben im string*/
				foreach(char bs in temp){ /*überprüfe alle Zeichen, ob sie Buchstaben sind*/
					if (Char.IsLetter (bs)) {
		...
		public void editObjekt(int objnr){
			foreach (Objekt o in objListe) {
				if (o.getObjNr () == objnr) {
